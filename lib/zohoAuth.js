...
// Builds a direct link to a record in the Zoho CRM web UI. Uses the
// org-less permalink form (no numeric org ID needed) - Zoho resolves it to
// the signed-in user's org automatically. apiDomain -> UI domain mapping:
// www.zohoapis.eu -> crm.zoho.eu, www.zohoapis.com -> crm.zoho.com, etc.
export function getCrmRecordUrl(apiDomain, module, recordId) {
  if (!apiDomain || !recordId) return null;
  const crmDomain = apiDomain.replace('https://www.zohoapis', 'https://crm.zoho');
  return `${crmDomain}/crm/tab/${module}/${recordId}`;
}
